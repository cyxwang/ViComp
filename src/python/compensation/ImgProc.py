...
def thresh(im_in):
    ...
    return im_mask, corners,bbox

def getLargestRect(immask):
    immask = immask.astype(np.uint8)
    contours,cnts = cv.findContours(immask,cv.RETR_EXTERNAL,cv.CHAIN_APPROX_SIMPLE) 
    rect = cv.minAreaRect(contours[0])
    
    points = cv.boxPoints(rect)
    points = np.int0(points)
    # bbox = [points[0][0],points[0][1]]
    c = contours[0] # sorted(cnts,key=cv.contourArea,reverse=True)[0]
    rect = order_points(c.reshape(c.shape[0],2))
   

    x = max(rect[0][0],rect[3][0])
    y = max(rect[0][1],rect[1][1])
    w = min(rect[1][0],rect[2][0])-x
    h = min(rect[3][1],rect[2][1])-y
